...
EPSILON_DECAY_STEPS =  (INITIAL_EPSILON-FINAL_EPSILON)/50000
...
NUM_EPISODES = 4000  # Episode limitation
...
def setParams():
    global GAMMA,INITIAL_EPSILON,FINAL_EPSILON,EPSILON_DECAY_STEPS,REPLAY_SIZE,BATCH_SIZE,TEST_FREQUENCY,SAVE_FREQUENCY,NUM_EPISODES,EP_MAX_STEPS,NUM_TEST_EPS,HIDDEN_NODES,learningRate
...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon, A_BOUND
    replay_buffer = []
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    #layer 1
    W1shape = [state_dim, 32]
    w1 = tf.get_variable("W1", shape=W1shape,initializer = tf.contrib.layers.xavier_initializer())
    initial = tf.constant(0.1, shape=[hidden_nodes])
    bshape = [1, 32]
    b1 = tf.get_variable("b1", shape=bshape, initializer = tf.contrib.layers.xavier_initializer())
    y1 = tf.nn.tanh(tf.matmul(state_in, w1) + b1)

    W2shape = [32, 128]
    w2 = tf.get_variable("W2", shape=W2shape,initializer = tf.contrib.layers.xavier_initializer())
    initial = tf.constant(0.1, shape=[hidden_nodes])
    bshape = [1, 128]
    b2 = tf.get_variable("b2", shape=bshape, initializer = tf.contrib.layers.xavier_initializer())
    y2 = tf.nn.relu(tf.matmul(y1, w2) + b2)

    W3shape = [128, 200]
    W3 = tf.get_variable("W3", shape=W3shape,initializer = tf.contrib.layers.xavier_initializer())
    bshape = [1, 200]
    b3 = tf.get_variable("b3", shape=bshape, initializer = tf.contrib.layers.xavier_initializer())
    y3 = tf.nn.relu(tf.matmul(y2, W3) + b3)

    W4shape = [200, action_dim]
    W4 = tf.get_variable("W4", shape=W4shape,initializer = tf.contrib.layers.xavier_initializer())
    bshape = [1, action_dim]
    b4 = tf.get_variable("b4", shape=bshape, initializer = tf.contrib.layers.xavier_initializer())
    ...
    optimise_step = tf.train.AdamOptimizer(learningRate).minimize(loss)
...
def get_env_action(action):
    ...
    if not isContinuous:
        return action
    f_action =  np.clip(action,A_BOUND[0],A_BOUND[1])
